...
```bash
files2cache$ sam local invoke HelloWorldFunction --event events/event.json
```
...
```bash
files2cache$ sam logs -n HelloWorldFunction --stack-name "files2cache" --tail
```
...
```bash
files2cache$ pip install pytest pytest-mock --user
files2cache$ python -m pytest tests/ -v
```
...
```bash
sam delete --stack-name "files2cache"
```
